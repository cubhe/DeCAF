...
class DeCAF(nn.Module):
    def __init__(self, FLAGS,net_kargs=None, name="model_summary"):
        ...
        self.lineares=nn.ModuleList([nn.Linear(FLAGS.mlp_kernel_size,FLAGS.mlp_kernel_size) for i in range(FLAGS.mlp_skip_layer[0])])
    ...
    def forward(self, coordinates, Hreal, Himag, padding, mask, reuse=False):
    ...
    def __forward_op(self,x,Hreal,Himag,padding):
        ...
        padded_field = F.pad(x, (0,0,58,58,58,58)) #orign
        #padded_field = F.pad(x, (0,0,4,4,4,4))
        padded_phase = padded_field[:, :, :, :, 0]
        padded_absorption = padded_field[:, :, :, :, 1]
        # transferred_field = torch.fft.ifft2(
        #     torch.mul(Hreal, torch.fft.fft2(padded_phase))
        #     + torch.mul(Himag, torch.fft.fft2(padded_absorption))
        # )
        # transferred_field = torch.fft.ifft2(
