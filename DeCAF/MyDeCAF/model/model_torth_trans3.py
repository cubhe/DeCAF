    ...
    def train(self, output_path, train_provider, learning_rate=1e-5, epochs=80):
        ...
        for epoch in range(FLAGS.start_epoch, epochs):
            ...
            for iter in range(iters_per_epoch):
                ...
                optimizer.zero_grad()
                _, xhats, Hxhats, loss, mse, ph_reg, ab_reg = self.__tower_loss(
                    0, self.Hreal_update_placeholder, self.Himag_update_placeholder, reuse=False
                )
                loss.backward()
                optimizer.step()
            ...
            if (epoch + 1) % FLAGS.model_save_epoch == 0:
                self.save(output_path, epoch=epoch + 1)
            if (epoch + 1) % FLAGS.intermediate_result_save_epoch == 0:
                self.save(output_path, train_provider=train_provider)
            else:
